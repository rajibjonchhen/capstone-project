...
function Products() {
        ...
        try {
            ...
            if(response.status !== 200){
                ...
                if(selectedCategory?.length > 0 && selectedCategory !== "all" ){
                    const products = data.products.filter(product => product.category === selectedCategory)
    ...
    return ( 
    <Container fluid >
        <div style={{position:"sticky", top:'60px', zIndex:'2', padding:'20px 0 5px'}}>
       ...
       <Row >
        ...
        <Row >
            ...
            style={{width:'150px'}}
            name={category.type}
            onClick = {(e) => {changeCategory(category.type)}}
            className="theme-btn">
            {category.title}
          </Button>
            )}
        ...
        <Row style={{margin:'50px auto'}} >
            ...
            { error.length > 0? <div className="error-message">Error on fetching products</div> : isLoading? <Spinner animation="border" variant="primary"/> : allProducts?.length === 0? <h2>There are no products</h2> : allProducts.map((product, i) => <Col key={i} item xs={12} sm={6} md={4} lg={3} style={{display:"flex", justifyContent:"center"}}>
